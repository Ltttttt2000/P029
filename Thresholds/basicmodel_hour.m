...
function [t,r,withoutpolicy,betachange,daysofpolicy, dayswithoutpolicy] = basicmodel_hour(tspan,yI,N, Ic,beta0,eplison)
...
for k = 1:N   %time h:day
    ...
    without = without(:,1)+h*odefunction(without(:,1),beta0);
    y(:,1) = y(:,1)+h*odefunction(y(:,1),beta);

    fprintf('Hour %d S0: %.2f I0: %.2f initial infected rate: %.2f S:%.2f I:%.2f infected rate:%.2f\n', ...
